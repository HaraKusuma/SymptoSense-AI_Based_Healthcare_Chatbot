...
# Updated prompt for adaptive style
template = """
You are Sympto Sense AI — a friendly and helpful medical chatbot specialized in symptom monitoring and disease information.
...
print("Sympto Sense AI RAG Chatbot is ready!")
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                if not text.strip():
                    flash("No text found in PDF. Try a different file.")
                    return redirect(request.url)
                ...
                flash("Report uploaded and indexed successfully!")

                # Generate summary & analysis
                summary_prompt = f"""
You are an expert doctor analyzing a medical report.

Extract and summarize key findings from this report in a clear, patient-friendly way.

Report text:
{text[:8000]}  # Limit to avoid token overflow

Include:
- Main diagnosis or test results
- Important values (high/low)
